...
Linux 内核使用`platform_device`结构体表示`platfrom`设备，如果内核支持设备树的话，就不需要使用`platform_device`来描述设备，改用设备树来描述，

### 相关结构体

#### `struct platform_device`

`platform_device`结构体定义在`include/linux/platform_device.h`中，结构体的内容如下：
...
`platform_device`的建立包含两种方式：

1. 在内核启动时通过 `device_node` 转换为 `platform_device`，将设备树中的节点转化为`platform_device`；
2. 使用`platform_device_register()`注册`platform_device`；
